...
public class ClickHouseQueryIml implements QueryService{
    ...
    public ClickHouseQueryIml(Connection clickHouseConn) {
      ...
      * 根据规则条件时间查询
      */
    public long eventActionQueryCount(String deviceId, EventParam eventParam) throws Exception {

        return eventActionQueryCount(deviceId,eventParam,eventParam.getTimeRangeStart(),eventParam.getTimeRangeEnd());

    }
    /*
     *根据分界点查询click house
     */
    public long eventActionQueryCount(String deviceId, EventParam eventParam, long queryStart, long queryEnd) throws Exception {

        PreparedStatement pst = clickHouseConn.prepareStatement(eventParam.getQuerySql());
